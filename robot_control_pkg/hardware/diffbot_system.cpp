  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  // Wait for Arduino to finish reset after DTR toggle
  ...
  const rclcpp::Time & /*time*/, const rclcpp::Duration & /*period*/)
{
  ...
  // Convert rad/s to ticks per frame for the Arduino PID controller
  // ticks_per_frame = rad/s ÷ (2π) × enc_counts_per_rev ÷ loop_rate
  double ticks_per_frame = cfg_.enc_counts_per_rev / (2.0 * M_PI * cfg_.loop_rate);
  int motor_l = static_cast<int>(wheel_l_.cmd * ticks_per_frame);
  int motor_r = static_cast<int>(wheel_r_.cmd * ticks_per_frame);
